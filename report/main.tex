...
\begin{titlepage}
  ...
  \begin{tabular}{ll}
    ...
    \textbf{Model}    & XGBoost + GridSearchCV (recall-optimised)               \\
    \textbf{Stack}    & Python · XGBoost · scikit-learn · imbalanced-learn · Streamlit \\
...
\textbf{Logistic Regression} as a linear baseline and an \textbf{XGBoost}
gradient-boosted tree ensemble as the primary classifier.  The training pipeline
optimises for recall on the minority (bad-loan) class via five-fold
cross-validated \textbf{GridSearchCV}, achieving an accuracy of approximately
$76\%$ and an area under the ROC curve (AUC) of approximately $0.80$ on the
held-out test set.  An interactive \textbf{Streamlit} dashboard surfaces the
model's predictions, default-probability scores, and risk indicators to end
users in real time.
...
\textbf{Keywords:} credit risk, logistic regression, XGBoost, gradient boosting,
...
\begin{tikzpicture}[
  ...
  \node[pipe,   below=of pre]   (train)  {3.\ GridSearchCV\\{\scriptsize XGBoost · 5-fold CV · Recall}};
  \node[pipe,   below=of train] (eval)   {4.\ Evaluate\\{\scriptsize Acc · AUC · $F_1$}};
  \node[store,  below=of eval]  (pkl)    {\footnotesize\code{credit\_risk\_model\_v2.pkl}};

  %% Training lane background
  \begin{scope}[on background layer]
    \node[lane=primary!6,
          fit=(csv)(load)(pre)(train)(eval)(pkl),
  ...
  %% ── TRAINING ARROWS ──────────────────────────────────────────────────────
  \foreach \a/\b in {csv/load, load/pre, pre/train, train/eval, eval/pkl}{
...
\caption{End-to-end system architecture. The \textcolor{primary}{\textbf{left lane}} is the
  offline training pipeline (runs once via \code{run\_training.py}), which trains an
  XGBoost classifier via 5-fold GridSearchCV; the
...
classifier to maximise accuracy by predicting the majority class.  The pipeline
addresses this in two complementary ways:

\begin{enumerate}
  \item \textbf{Recall-optimised GridSearchCV.}  The hyperparameter search uses
        \texttt{scoring='recall'}, so every candidate model is selected based on
        its ability to identify bad-loan applicants (true positive rate on the
        minority class), not on raw accuracy.

  \item \textbf{imbalanced-learn Pipeline.}  The training pipeline is constructed
        with \texttt{imblearn.Pipeline}, which is drop-in compatible with
        scikit-learn's cross-validation infrastructure and is designed to
        accommodate resampling steps (e.g., SMOTE) between folds without data
        leakage.  In the current implementation the pipeline contains the
        preprocessor and classifier steps; a SMOTE resampler can be inserted
        as a third step in future iterations without altering any downstream code.
\end{enumerate}

Figure~\ref{fig:smote} illustrates the effect of the imbalanced distribution
and the recall-scoring objective.
...
\begin{tikzpicture}
  \begin{axis}[
    ...
    \addplot[fill=primary!70, draw=primary] coordinates {(Good (0),560) (Bad (1),240)};
    \legend{Training set (80\,\% split)}
  \end{axis}
\end{tikzpicture}
\caption{Class distribution in the training split ($n=800$): $560$ good-credit
  and $240$ bad-credit samples.  GridSearchCV optimises for \emph{recall} on
  the bad class to counteract this imbalance.}
\label{fig:smote}
\end{figure}

\subsection{Model: XGBoost}
\label{sec:rf}

We select \textbf{XGBoost}~\cite{chen2016} (Extreme Gradient Boosting) as the
primary classifier, a gradient-boosted tree ensemble that iteratively adds
weak learners to correct residual errors.

\subsubsection{Mathematical Foundation}

\begin{definition}[Gradient Boosting Objective]
  XGBoost minimises a regularised objective over an additive ensemble of $T$
  trees $\{f_1, \ldots, f_T\}$:
  \begin{equation}
    \mathcal{L}(\phi) = \sum_{i=1}^{n} l\!\left(y^{(i)},\, \hat{y}^{(i)}\right)
      + \sum_{t=1}^{T} \Omega(f_t),
    \label{eq:xgb-obj}
  \end{equation}
  where $l$ is the binary log-loss and
  $\Omega(f) = \gamma T_{\text{leaves}} + \tfrac{1}{2}\lambda\|w\|^2$
  penalises tree complexity.
\end{definition}

\begin{definition}[Additive Training Step]
  At boosting round $t$, a new tree $f_t$ is fitted to the second-order
  Taylor approximation of the loss:
  \begin{equation}
    \tilde{\mathcal{L}}^{(t)} \approx \sum_{i=1}^{n}
      \Bigl[g_i f_t(\bfmath{x}^{(i)}) + \tfrac{1}{2}\,h_i f_t^2(\bfmath{x}^{(i)})\Bigr]
      + \Omega(f_t),
    \label{eq:xgb-taylor}
  \end{equation}
  where $g_i = \partial_{\hat{y}^{(t-1)}} l(y^{(i)}, \hat{y}^{(t-1)})$ and
  $h_i = \partial^2_{\hat{y}^{(t-1)}} l(y^{(i)}, \hat{y}^{(t-1)})$ are the
  first and second gradients of the loss.
\end{definition}

\begin{definition}[XGBoost Default Probability]
  The model output is the sum of tree scores mapped through the sigmoid:
  \begin{equation}
    \hat{p}_{\text{bad}}(\bfmath{x}) =
      \sigma\!\left(\sum_{t=1}^{T} f_t(\bfmath{x})\right)
      = \frac{1}{1 + e^{-\sum_{t=1}^{T} f_t(\bfmath{x})}}.
    \label{eq:xgb-prob}
  \end{equation}
  The binary prediction is $\hat{y} = \mathbb{1}[\hat{p}_{\text{bad}} \geq 0.5]$.
\end{definition}
...
\begin{table}[H]
  \centering
  \caption{GridSearchCV Hyperparameter Grid (XGBoost)}
  \label{tab:grid}
  \begin{tabular}{lll}
    \toprule
    \textbf{Parameter}                    & \textbf{Candidates}      & \textbf{Rationale} \\
    \midrule
    \code{n\_estimators}                  & \{100, 200\}             & Boosting rounds \\
    \code{max\_depth}                     & \{3, 6, 10\}             & Tree depth / bias–variance \\
    \code{learning\_rate}                 & \{0.01, 0.1\}            & Shrinkage (step size) \\
    \code{min\_child\_weight}             & \{1, 5\}                 & Minimum leaf coverage \\
    \code{subsample}                      & \{0.8, 1.0\}             & Row sub-sampling \\
    \code{colsample\_bytree}              & \{0.8, 1.0\}             & Column sub-sampling \\
    \bottomrule
  \end{tabular}
\end{table}
...
(compatible with scikit-learn's cross-validation infrastructure):
\[
  \mathcal{P} = \bigl[\,
    \underbrace{\text{ColumnTransformer}}_{\text{preprocess}},\;
    \underbrace{\text{XGBClassifier}}_{\text{classify}}
  \,\bigr].
\]
Wrapping the pipeline in \texttt{imblearn.Pipeline} ensures compatibility
with future resampling steps (e.g., SMOTE) without data leakage across folds.
...
the XGBoost ensemble is benchmarked.  It estimates the default probability using
...
\begin{tikzpicture}[
  ...
  %% Row 2 (train branch): CT → XGBoost
  \node[pnode, below=1.5cm of split, xshift=-2.0cm] (ct)
        {ColumnTransformer\\\scriptsize StandardScaler + OHE};
  \node[mnode, right=of ct]   (rf) {XGBoost\\\scriptsize GridSearchCV};

  \draw[-{Stealth}, thick, primary]
    (split) -- node[lbl, right]{train $800\times9$} (ct);
  \draw[-{Stealth}, thick, primary]
    (ct) -- node[lbl, above]{$\mathbb{R}^{800\times d}$} (rf);
  ...
  \begin{scope}[on background layer]
    \node[fill=primary!5, rounded corners=8pt,
          fit=(ct)(rf)(ev)(pkl), inner sep=6pt,
...
The optimised XGBoost model outperformed the Logistic Regression baseline
...
\begin{table}[H]
  ...
  \begin{tabular}{lccc}
    ...
    XGBoost (default params)             & 0.74 & 0.60 & 0.78 \\
    \midrule
    \rowcolor{primary!8}
    \textbf{XGBoost — Full Pipeline (v2)} & \textbf{0.76} & \textbf{0.68} & \textbf{0.80} \\
    \midrule
...
an XGBoost model with default parameters, and the final GridSearchCV-tuned
pipeline.  The LR model's recall of $0.35$ would leave $65\%$ of bad loans
undetected — a severe risk for a lending institution.
...
\begin{tikzpicture}
  \begin{axis}[
    ...
    %% XGBoost ROC (approximate, illustrative)
    ...
    \addlegendentry{XGBoost (AUC $\approx$ 0.80)}
  ...
  \end{axis}
\end{tikzpicture}
\caption{ROC curve for the XGBoost classifier. The curve is well above the
...
Figure~\ref{fig:fi} shows the XGBoost \emph{gain}-based feature importance for the top features (average gain across all splits that use each feature).
...
\begin{tikzpicture}
  \begin{axis}[
    ...
    ytick={SexMale, HousingOwn, PurposeCar, CheckingLittle,
           SavingLittle, JobSkill, Duration, Age, CreditAmount},
    yticklabels={
      Sex: Male,
      Housing: Own,
      Purpose: Car,
  ...
  \end{axis}
\end{tikzpicture}
\caption{Top-9 feature importances ranked by XGBoost gain (average gain of
  all tree splits that use each feature).
...
status captures immediate liquidity, a strong short-term repayment signal.

\subsection{Class Distribution in Training Data}
...
\begin{tikzpicture}
  \begin{axis}[
    ...
    tick label style={font=\small},
    label style={font=\small\bfseries},
    title={Class Distribution: Full Dataset vs.\ Training Split},
    title style={font=\bfseries},
    nodes near coords,
    nodes near coords align={vertical},
  ]
    \addplot[fill=primary!70,  draw=primary]  coordinates {(Good (0),700) (Bad (1),300)};
    \addplot[fill=success!70,  draw=success]  coordinates {(Good (0),560) (Bad (1),240)};
    \legend{Full dataset ($n=1000$), Training split ($n=800$)}
  \end{axis}
\end{tikzpicture}
\caption{Class distribution in the full dataset ($7{:}3$ ratio) and the
  stratified 80\,\% training split.  The imbalance is preserved in the split;
  GridSearchCV with \texttt{scoring='recall'} compensates at training time.}
\label{fig:smote-bar}
\end{figure}

\subsection{Decision Tree Diagram (Single XGBoost Estimator)}

Figure~\ref{fig:tree} shows a simplified decision tree (depth 3) representative
of a single weak learner inside the XGBoost ensemble, illustrating the
splitting logic.
...
\caption{Simplified depth-3 decision tree illustrating the primary splitting
  rules, representative of a single weak learner in the XGBoost ensemble.
  Large loan amounts combined with older applicant age increase the
...
\begin{table}[H]
  ...
  \begin{tabular}{lll}
    ...
    \midrule
    Classifier            & XGBoost 3.x             & Gradient-boosted tree ensemble \\
    ML Pipeline           & scikit-learn 1.x        & Preprocessing (scaler, OHE) \\
    Pipeline Wrapper      & imbalanced-learn        & Resampling-compatible pipeline \\
...
\begin{enumerate}
  ...
  \item \textbf{Prediction.} The XGBoost ensemble returns:
        \begin{itemize}
...
\begin{itemize}
  \item A \textbf{robust training pipeline} with standard scaling, one-hot
        encoding, and 5-fold cross-validated XGBoost tuning optimised for
        recall on the minority (bad-loan) class.
...
\begin{enumerate}
  ...
  \item The pipeline does not currently include a resampling step; the class
        imbalance is addressed solely through recall-optimised GridSearchCV,
        which may still leave some minority-class bias.
  \item The model is not calibrated — the raw sigmoid outputs may not match
        empirical default rates precisely.
\end{enumerate}
...
\begin{enumerate}
  \item \textbf{Milestone 2 — Agentic AI Lending Assistant.}
        ...
        lending rules.
  \item \textbf{Resampling.}  Integrate SMOTE or class-weight tuning inside
        the imbalanced-learn pipeline to further improve bad-class recall beyond
        the recall-scoring approach currently used.
  \item \textbf{Advanced Boosting.}  Benchmark LightGBM and CatBoost against
        the current XGBoost model; ensemble stacking may yield further gains.
